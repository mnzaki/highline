        ...
        if JRUBY                      # if we are on JRuby. JLine is bundled with JRuby.
          ...
          def initialize(input, output)
            if JRUBY_VERSION =~ /^1.7/
              ...
              @java_input = instream = input.nil? ? nil : input.to_inputstream
              @java_output = output.nil? ? nil : output.to_outputstream
              ...
              @java_input = input.nil? ? nil :
                Channels.newInputStream(input.to_channel)
              @java_output = output.nil? ? nil :
                OutputStreamWriter.new(Channels.newOutputStream(output.to_channel))
